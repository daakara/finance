...
export default function JournalWorkbench() {
  ...
  const { status, errorMessage, triad, calibrationScore, identityDrift } = state;
    ...
    <main className="min-h-screen bg-[#070b12] text-gray-100 p-4 md:p-8 space-y-8 max-w-7xl mx-auto">
      {status === 'ERROR' && (
        <div className="p-4 rounded-xl bg-rose-950/40 border border-rose-800 text-xs font-mono text-rose-300 flex items-center justify-between">
          <span>Failed to load journal calibration telemetry: {errorMessage || "Network error"}</span>
          <button
      ...
      {status === 'LOADING' && (
        <div className="p-3 rounded-lg bg-indigo-950/30 border border-indigo-800/40 text-xs font-mono text-indigo-300 animate-pulse">
          Synchronizing calibration scores and drift alerts...
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="p-6 rounded-xl bg-gray-900/60 border border-gray-800 space-y-2">
          ...
          <p className="text-xs text-gray-400">Bias status: {calibrationScore?.overconfidenceBias ?? "Nominal"} ({calibrationScore?.trend ?? "Stable"}).</p>
        </div>

        <div className="p-6 rounded-xl bg-gray-900/60 border border-gray-800 space-y-2">
          <span className="text-xs font-mono uppercase text-gray-400">Active Drift Alert</span>
          <p className="text-lg font-mono font-bold text-amber-400">{identityDrift?.domain ?? "None"}</p>
          <p className="text-xs text-gray-400">{identityDrift ? `${identityDrift.inactiveDays}d inactive • ${identityDrift.remedyAction}.` : "Zero active drift detected across declared domains."}</p>
        </div>
      </div>
    </main>
  );
