...
export default function SignalsWorkbench() {
  ...
  const { triad, signalQuality, recoveryIndicator } = state;
    ...
    <main className="min-h-screen bg-[#070b12] text-gray-100 p-4 md:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 rounded-xl bg-gray-900/60 border border-gray-800 space-y-3">
          ...
          <p className="text-2xl font-mono font-bold text-emerald-400">{signalQuality.freshness}</p>
          <p className="text-xs text-gray-400">{signalQuality.activeSignalsCount} active telemetry streams ({signalQuality.confidence}% confidence).</p>
        </div>

        <div className="p-6 rounded-xl bg-gray-900/60 border border-gray-800 space-y-3">
          <span className="text-xs font-mono uppercase text-gray-400">Autonomic Recovery</span>
          <p className="text-2xl font-mono font-bold text-cyan-400">{recoveryIndicator.hrvTrend}</p>
